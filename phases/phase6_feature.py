...
def process_image_wrapper(args):
    ...
    try:
        ...
        return (fname, cls, label_idx, split_type, feats)
...
def collect_all_tasks(data_dir, test_base_dir, test_per_class):
    ...
    for label_idx, cls in enumerate(CLASSES):
        ...
        if not os.path.isdir(class_dir):
            print(f"[WARNING] train class folder not found: {class_dir}")
            continue
    ...
    for label_idx, cls in enumerate(CLASSES):
        ...
        if not os.path.isdir(test_class_dir):
            print(f"[WARNING] test class folder not found: {test_class_dir}")
            continue
        ...
        test_files = sorted(test_files)[:test_per_class]

        for fname in test_files:
            full_path = os.path.join(test_class_dir, fname)
            tasks.append((full_path, fname, cls, label_idx, "test"))
...
def build_feature_csv(data_dir, test_base_dir, out_csv="feature100.csv", test_per_class=25):
    ...
    with open(out_csv, "w", newline="", encoding="utf-8") as f:
        ...
        for fname, cls, label_idx, split_type, feats in results:
            row = [fname, cls, label_idx, split_type] + feats.tolist()
            writer.writerow(row)

    print(f"[INFO] features saved to {out_csv}")
    print(f"[INFO] Total rows written: {len(results)}")


if __name__ == "__main__":
    build_feature_csv(DATA_DIR, TEST_BASE_DIR, out_csv="feature100.csv", test_per_class=TEST_IMAGES_PER_CLASS)
