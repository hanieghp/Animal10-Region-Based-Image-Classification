...
def compute_intensity_stats(gray):
    ...
    flat = g.ravel()
    if flat.size > 1:
        skewness_val = float(skew(flat, bias=False))
        kurtosis_val = float(kurtosis(flat, fisher=True, bias=False))
    else:
        skewness_val = 0.0
        kurtosis_val = 0.0
...
def process_image(full_path):
    ...
    width, height, aspect_ratio = compute_basic_dims(img_orig)
    ...
    # فیچرها به ترتیب همان هدر (از width به بعد)
    feats = [
        width,
        height,
        aspect_ratio,
     ...
     # ==== این سه خط جدید است: حذف NaN و inf ====
